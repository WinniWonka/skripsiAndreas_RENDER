...
def Calc_Dist(rfm_df_scaled, m, f, r, Centroid_df, k):
    for i in range(k):
        rfm_df_scaled[str(i)] = np.sqrt((m - Centroid_df.iloc[i, 0]) ** 2 +
                                        (f - Centroid_df.iloc[i, 1]) ** 2 + (r - Centroid_df.iloc[i, 2]) ** 2)
...
def df_membership(rfm_df_scaled2, k):
    clt = k
    index = 1
    rfm_df_scaled2['Index Cluster'] = (rfm_df_scaled2.loc[:, ['{}'.format(
        index) for index in range(clt)]].idxmin(axis=1)).astype('int')
...
def main():
    ...
    if menu == 'Home':
        ...
        # Setup file upload
        retail = st.sidebar.file_uploader(label='Support CSV', type=[
        ...
        if retail is not None:
            ...
            k = st.sidebar.slider(min_value=2, max_value=10,
                                  value=2, label='Number of Clusters')
            ...
            if (st.sidebar.button(label='Hierarchical K-Means Clustering', help='for clustering', on_click=Clt_Res_Callback) or st.session_state.Clt_Res):
                ...
                IQR = Q3 - Q1
                rfm = rfm[(rfm.Amount >= Q1 - 1.5*IQR) &
                          (rfm.Amount <= Q3 + 1.5*IQR)]
                ...
                # Single Linkage
                _hierarchicalClustering = linkage(
                    rfm_df_scaled, method=hierarchical_method, metric='euclidean')
                cutTree = cut_tree(_hierarchicalClustering,
                                   n_clusters=k).reshape(-1,)

                # Tab function
                tab1, tab2, tab3, tab4 = st.tabs(
                    ['Dendrogram Hierarchical Clustering', 'Silhouette Score', 'RFM Boxplot', 'RFM Analysis'])
                # Dendogram
                with tab1:
                ...
                rfm_df_scaled2 = Calc_Dist(
                    rfm_df_scaled, rfm_df_scaled['Amount'], rfm_df_scaled['Frequency'], rfm_df_scaled['Recency'], Centroid_df, k)

                # for Index Cluster
                rfm_df_scaled2 = df_membership(rfm_df_scaled2, k)

                # Final HKMeans
                rfm_df_scaled2, centroid_df, iterasi = clusterisasi(
                    rfm_df_scaled, rfm_df_scaled['Amount'], rfm_df_scaled['Frequency'], rfm_df_scaled['Recency'], k, Centroid_df)
                ...
                with tab2:
                    ...
                    for num_clusters in range_n_clusters:
                        ...
                        ax1.set_ylim(
                            [0, len(rfm_df_scaled_sil) + (num_clusters + 1) * 10])

                        _hierarchicalClustering_sil = linkage(
                            rfm_df_scaled_sil, method=hierarchical_method, metric='euclidean')
                        cutTree_sil = cut_tree(
                            _hierarchicalClustering_sil, n_clusters=num_clusters).reshape(-1,)
                        rfm_df_scaled_sil['Cluster Hierarchical'] = cutTree_sil

                        Centroid_df_sil = Initiate_Centroid(
                            rfm_df_scaled_sil, num_clusters)
                        rfm_df_scaled_sil2, Centroid_df_sil, iterasi_sil = clusterisasi(
                            rfm_df_scaled_sil, rfm_df_scaled_sil['Amount'], rfm_df_scaled_sil['Frequency'], rfm_df_scaled_sil['Recency'], num_clusters, Centroid_df_sil)
                        ...
                        silhouette_avg = silhouette_score(
                            rfm_df_scaled_sil2, cluster_labels_sil)
                        ...
                        ax1.set_title(
                            "The silhouette plot for number of clusters {0}.".format(num_clusters))
                        ax1.set_xlabel("The silhouette coefficient values")
                        ax1.set_ylabel("Cluster label")

                        # The vertical line for average silhouette score of all the values
                        ax1.axvline(x=silhouette_avg,
                                    color="red", linestyle="--")
                    ...
                    silhouette_opt = silhouette_df.loc[silhouette_df['Silhouette Score']
                                                       == silhouette_max]
                ...
                with tab3:
                    ...
                    sns.boxplot(x='Hk Means Cluster', y='Recency',
                                data=rfm).set_title('Recency Plot')
                    st.pyplot(fig_rec)
                    ...
                    sns.boxplot(x='Hk Means Cluster', y='Frequency',
                                data=rfm).set_title('Frequency Plot')
                    st.pyplot(fig_freq)
                    ...
                    sns.boxplot(x='Hk Means Cluster', y='Amount',
                                data=rfm).set_title('Amount Plot')
                    st.pyplot(fig_amt)
                ...
                with tab4:
                    ...
                    st.image(rfm_img)
                    st.markdown(
                        "<p style='text-align: center;'>Customer Value Matrixes (Der-Chiang Li, Wen-Li Dai, and Wan-Ting Tseng, 2011)</p>", unsafe_allow_html=True)
                    with st.expander('See Explanation !'):
                        st.markdown(
                            "<p style='text-align: left;'><strong>Spenders</strong> class, are customers who have demonstrated a capacity for high average purchase amounts.</p>", unsafe_allow_html=True)
                        st.markdown(
                            "<p style='text-align: left;'><strong>Frequent</strong> class, are customers who have proven their loyalty via repeat purchases.</p>", unsafe_allow_html=True)
                        st.markdown(
                            "<p style='text-align: left;'><strong>Uncertain</strong> class, are customers who have low purchasing power and interest.</p>", unsafe_allow_html=True)
                        st.markdown(
                            "<p style='text-align: left;'><strong>Best</strong> class, are customers who have high average purchase amounts and frequency</p>", unsafe_allow_html=True)
                    ...
                    with col2:
                        ...
                        for num_clusters in range(k):
                            ...
                            if stdDev_df.iloc[num_clusters, 1] > avg_f:
                                f_output = 'Frequency at cluster {0} is F(\u2191)'.format(
                                    num_clusters)
                                f_flag = 'F_UP'
                            else:
                                f_output = 'Frequency at cluster {0} is F(\u2193)'.format(
                    ...
                    with col3:
                        ...
                        for num_clusters in range(k):
                            ...
                            if stdDev_df.iloc[num_clusters, 0] > avg_m:
                                m_output = 'Monetary at cluster {0} is M(\u2191)'.format(
                                    num_clusters)
                                m_flag = 'M_UP'
                            else:
                                m_output = 'Monetary at cluster {0} is M(\u2193)'.format(
                    ...
                    with col4:
                        ...
                        for i in range(k):
                            ...
                            if (stdDev_df.iloc[i, 3] == 'F_DOWN' and stdDev_df.iloc[i, 4] == 'M_UP'):
                                mapping_rfm = 'then cluster {0} is Spender'.format(
                                    i)
                            elif (stdDev_df.iloc[i, 3] == 'F_UP' and stdDev_df.iloc[i, 4] == 'M_UP'):
                                mapping_rfm = 'then cluster {0} is Best'.format(
                                    i)
                            elif (stdDev_df.iloc[i, 3] == 'F_DOWN' and stdDev_df.iloc[i, 4] == 'M_DOWN'):
                                mapping_rfm = 'then cluster {0} is Uncertain'.format(
                                    i)
                            elif (stdDev_df.iloc[i, 3] == 'F_UP' and stdDev_df.iloc[i, 4] == 'M_DOWN'):
                                mapping_rfm = 'then cluster {0} is Frequent'.format(
                                    i)
                ...
                if st.sidebar.checkbox(label='Show Data'):
                    ...
                    st.sidebar.download_button(
                        label='Save Data', data=csv_RFM, file_name='RFM_Hkmeans.csv', mime='text/csv')

    if menu == 'Guide':
        st.markdown(
            "<h2 style='text-align: center;'>User Manual Guide</h2>", unsafe_allow_html=True)
        ...
        st.markdown("""<a href="https://drive.google.com/file/d/1ThIziGLt0FBRqtRHChLEX7XhaOmolVE9/view?usp=sharing">OnlineRetail.csv</a>""", unsafe_allow_html=True)

        # Home
        st.subheader('1. Home')
        home_img = ImageOps.expand(Image.open(
            'img/user_manual/home.png'), border=5, fill='white')
        ...
        uploadData_img = ImageOps.expand(Image.open(
            'img/user_manual/uploadDataset.png'), border=5, fill='white')
        ...
        uploadDataAfter_img = ImageOps.expand(Image.open(
            'img/user_manual/uploadDataset_after.png'), border=5, fill='white')
        st.image(uploadDataAfter_img, caption='After Upload Dataset')

        # Number of Clusters
        st.subheader('1.2. Number of Clusters')
        slider_numClt_img = ImageOps.expand(Image.open(
            'img/user_manual/slider_numClt.png'), border=5, fill='white')
        ...
        hierarchicalClustering_img = ImageOps.expand(Image.open(
            'img/user_manual/hierarchicalClustering.png'), border=5, fill='white')
        st.image(hierarchicalClustering_img,
        ...
        hierarchicalClustering_after_img = ImageOps.expand(Image.open(
            'img/user_manual/hierarchicalClustering_after.png'), border=5, fill='white')
        st.image(hierarchicalClustering_after_img,
                 caption='After Clicking Hierarchical K-Means Clustering')

        # Show Data
        st.subheader('1.4. Show Data')
        showDataset_img = ImageOps.expand(Image.open(
            'img/user_manual/showDataset.png'), border=5, fill='white')
        ...
        showDatasetAfter_img = ImageOps.expand(Image.open(
            'img/user_manual/showDatasetAfter.png'), border=5, fill='white')
        st.image(showDatasetAfter_img, caption='Show Data Result')

        # Save Data
        st.subheader('1.5. Save Data')
        saveDataset_img = ImageOps.expand(Image.open(
            'img/user_manual/saveDataset.png'), border=5, fill='white')
    ...
    if menu == 'About':
        st.markdown(
            "<h2 style='text-align: center;'>About the Application</h2>", unsafe_allow_html=True)
        ...
        st.markdown(
            '<p style="text-align: justify;">The flowchart of the combination of these algorithms can be seen below</p>', unsafe_allow_html=True)
        flowchart_hkMeans_img = ImageOps.expand(Image.open(
            'img/user_manual/flowchart_hkMeans.png'), border=5, fill='white')
        ...
        st.markdown(
            "<h2 style='text-align: center;'>About the Author</h2>", unsafe_allow_html=True)
        ...
        st.markdown(
            '<p style="text-align: justify;">You can contact him through email : andreaslie19@gmail.com</p>', unsafe_allow_html=True)

        st.markdown("<h2 style='text-align: center;'>References</h2>",
